    ...
    async def _progress_updater(self):
        ...
        while self.running and consecutive_errors < max_errors:
            try:
                ...
                # Check if we have data and enough time has passed for Telegram API
                if (self.current_data and self.msg and 
                    (current_time - self.last_telegram_update) >= self.telegram_limit):
    ...
    def hook(self, d):
        ...
        try:
            if d["status"] == "downloading":
                ...
                now = time.time()
                if (self.chat_id not in last_update_time) or (now - last_update_time[self.chat_id] > 0.2):
