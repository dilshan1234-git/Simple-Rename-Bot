...
async def yt_callback_handler(bot, query):
    global download_queue, currently_downloading
    ...
    # If max queue size reached
    if len(download_queue) >= MAX_QUEUE:
        await query.answer("🚫 Queue is full (max 15). Try again later.", show_alert=True)
        return

    # Inform user: video is queued
    await query.message.edit_text(f"⏳ **Download queued...**\n\n**🎞 {title}**\n\n**📹 {resolution}**")

    # Add task to queue
    download_queue.append((chat_id, query, format_id, resolution, url))
...
async def process_queue(bot):
    ...
    if currently_downloading or not download_queue:
        return

    currently_downloading = True
    chat_id, query, format_id, resolution, url = download_queue.popleft()

    try:
        await perform_download(bot, chat_id, query, format_id, resolution, url)
    except Exception as e:
        await bot.send_message(chat_id, f"❌ **Unexpected Error:** {e}")
    finally:
        currently_downloading = False
        await process_queue(bot)  # Trigger next download


async def perform_download(bot, chat_id, query, format_id, resolution, url):
    title = query.message.caption.split('🎞 ')[1].split('\n')[0]
    ...
    try:
        ...
        await download_message.edit_text(f"❌ **Download failed:** {e}")
        return
    ...
    try:
        ...
        await uploading_message.edit_text(f"❌ **Upload failed:** {e}")
        return
